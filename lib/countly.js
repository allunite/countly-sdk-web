 ...
 */
	Countly.init = function(ob){
		if(!inited){
			...
			Countly.nest_url = ob.nest_url || Countly.nest_url || null;
			Countly.q = Countly.q || [];
    ...
    */
	...
	Countly.begin_session = function(noHeartBeat){
		if(!sessionStarted){
			...
			toRequestQueue(req);
			//TODO: Implement nest URL stting and iframe injection
			document.write()
		}
	};
    ...
    **/
    Countly.track_clicks = function(parent){
        ...
        add_event(parent, "mousedown", processClick);
        add_event(parent, "mouseup", processClick);
	...
	function getMetrics(){
        ...
        */
        ...
        for(var i = 0; i < browsers.length; i++){
            if (nAgt.indexOf(browsers[i][0]) != -1) {
                browser = browsers[i][1];
                break;
            }
        }
		
        if (!browser && (nameOffset = nAgt.lastIndexOf(' ') + 1) < (verOffset = nAgt.lastIndexOf('/'))) {
